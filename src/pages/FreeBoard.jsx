...
import { Search, PenSquare, MessageSquare, Eye, Clock, User, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';

// Mock data is no longer needed but kept as reference if no data exists
const mockPosts = [];
...
export default function FreeBoard() {
  ...
  useEffect(() => {
    // 쿼리 최적화: 최신 글 20개만 먼저 가져오고, 로컬 캐시를 우선 활용하도록 설정
    ...
    const unsubscribe = onSnapshot(q, { includeMetadataChanges: true }, (snapshot) => {
      ...
      // 최신순 정렬
      const sortedPosts = [...fetchedPosts].sort((a, b) => b.rawDate - a.rawDate);
      ...
      <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl w-full md:w-auto">
            {['전체', '질문', '정보', '수다'].map((cat) => (
              <button 
                ...
                className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeCategory === cat ? 'bg-white text-[#172554] shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
              >
                {cat}
              </button>
            ))}
          ...
          <div className="divide-y divide-slate-50 min-h-[400px]">
            {isLoading ? (
              ...
              <Link 
                ...
                className="grid grid-cols-1 md:grid-cols-[80px_1fr_150px_120px_100px] gap-4 px-6 py-5 hover:bg-slate-50/80 transition-colors cursor-pointer group"
              >
                {/* Mobile: Top Section */}
                ...
                <div className="py-20 text-center bg-white w-full h-full">
                  <p className="text-slate-400 font-medium font-outfit">등록된 게시글이 없습니다. 첫 번째 글의 주인공이 되어보세요!</p>
                </div>
              )}
          </div>
        </div>
        ...
        <div className="flex justify-center items-center mt-10 space-x-2">
          ...
          {[1].map((pageNum) => (
            <button 
              key={pageNum}
              className={`w-10 h-10 rounded-lg text-sm font-bold transition-all ${pageNum === 1 ? 'bg-[#172554] text-white shadow-md' : 'text-slate-600 hover:bg-white hover:text-[#172554]'}`}
            >
              {pageNum}
            </button>
          ))}
